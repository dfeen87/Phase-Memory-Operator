...
Hysteresis and path-dependent regime shifts occur across biological and engineering systems, including human-in-the-loop settings, where classical optimization and control strategies can fail under timing drift and geometric misalignment. We present a deterministic phase–memory operator that extracts a triadic latent state—phase deviation, drift, and accumulated memory—from observed signals and yields an interpretable stability functional for supervised diagnostic assessment of transition regimes. The proposed operator is model-agnostic, requires no retraining, and is designed to operate as an auditable supervisory diagnostic layer rather than as a controller or general-purpose predictive classifier. We outline a reproducible calibration and evaluation protocol and summarize an external validation on EEG time series demonstrating fixed-parameter replay, deterministic regime scoring, and transparent failure-mode analysis in collapse-like dynamics. Finally, we provide an application roadmap for extending the method to other hysteretic systems, including oxygen-delivery monitoring as a representative human-in-the-loop example, emphasizing falsification criteria, benchmark limitations, and transparent scientific assessment.

**Keywords:** phase–memory operator; hysteresis; path dependence; deterministic replay; supervisory diagnostics; regime-shift detection

---

## 1  Introduction

Hysteresis and path-dependent regime shifts arise in a wide range of complex systems, including physiological regulation, coupled human–machine loops, and engineering processes affected by drift, delay, and accumulated state history [4–6, 14, 19]. In such settings, identical instantaneous measurements may correspond to different effective system states depending on the preceding trajectory, which limits the usefulness of purely memoryless thresholding or static classification rules. Classical learning-based approaches can be effective for event detection and forecasting, but in safety-critical contexts they may be difficult to audit and replay exactly, and may remain sensitive to retraining or dataset-specific adaptation [1, 15].

Motivated by these constraints, the present work introduces a deterministic, runtime-evaluable phase–memory operator intended to function as a diagnostic/supervisory layer for regime-shift detection rather than as a control law. The aim is not to replace existing controllers or decision systems, but to provide an interpretable stability index that can be computed from observed signals under fixed and reviewable parameter settings.

**Contributions.** The main contributions of this work are as follows:

- A deterministic phase–memory operator that produces a triadic latent state (U₁, U₂, U₃) from observed time series.
- A stability functional and thresholding logic for supervised regime-instability assessment under drift, delay, and hysteretic separation.
- A reviewer-auditable calibration and validation protocol emphasising temporal leakage control, frozen parameters after calibration, and explicit falsification criteria.
...
1. phase-like structure or oscillatory organisation,
...
> z(t) = s(t) + i ℋ[s(t)],  &emsp;  θ(t) = arg z(t)  &emsp; (2)

where ℋ[·] denotes the Hilbert transform and θ(t) is the instantaneous phase associated with the preprocessed signal *s(t)*. A reference phase θᵣₑ꜀(t) is then specified from one of the following sources: a baseline segment, a nominal reference model, or an adaptive low-variance estimator. The chosen construction must be declared explicitly and kept fixed after calibration.
...
> U₁(t) := Δθ(t) = wrap(θ(t) − θᵣₑ꜀(t))  &emsp; (3)
>
> U₂(t) := Δω(t) = θ̇(t) − θ̇ᵣₑ꜀(t)  &emsp; (4)
>
> U₃(t) := m(t) = ∫₍ₜ₋ₜₘ₎ᵗ  w(τ) Δθ(τ) dτ,  &emsp;  Tₘ > 0  &emsp; (5)
...
> U₁[n] := wrap(θ[n] − θᵣₑ꜀[n])  &emsp; (6)
>
> U₂[n] := (θ[n] − θ[n−1]) / Δt  −  (θᵣₑ꜀[n] − θᵣₑ꜀[n−1]) / Δt  &emsp; (7)
>
> U₃[n] := Δt · Σₖ₌ₙ₋ₘₘ₊₁ⁿ  w[k] U₁[k]  &emsp; (8)
...
## 5  Stability Functional and Supervisory Alert Logic
...
A supervisory alert is issued when the score remains above a critical level J꜀ for a sustained dwell interval ΔT:

> Alert(t) = 1  ⟺  J(τ) ≥ J꜀  ∀ τ ∈ [t − ΔT, t]  &emsp; (10)

**Discrete alert logic.** In sampled form, with M_d = ⌊ΔT/Δt⌋, the alert rule becomes

> Alert[n] = 1  ⟺  J[k] ≥ J꜀  ∀ k ∈ {n − M_d + 1, …, n}  &emsp; (11)
...
- **Calibration window:** estimate θᵣₑ꜀(t), scale parameters (σ_θ, σ_ω, σ_m), weights (a, b, c), and choose (J꜀, ΔT, Tₘ).
...
**Table 2: Recommended evaluation metrics for supervisory regime-shift evaluation.**

| Metric | Definition / purpose |
|--------|----------------------|
| Lead time | Median time between first supervisory alert and event onset |
| ROC-AUC | Threshold-independent discrimination performance |
| PR-AUC | Performance under class imbalance |
| False alarm rate | Alert frequency per unit time in non-event windows |
...
| Signal preprocessing | Filter type/order, band limits, resampling rate, normalization, artifact masks; deterministic pipeline definition. | Locked | Pipeline config + hash |
...
| Reference phase θᵣₑ꜀(t) | Baseline window selection rules or nominal reference construction; update policy (if any) must be pre-declared. | Locked | Reference spec + indices |
...
| Alert parameters (J꜀, ΔT) | Threshold J꜀, dwell time ΔT, warning policy; any smoothing or hysteresis in alert logic pre-declared. | Locked | Alert policy config |
...
- preprocessing map 𝒫,
- phase-reference construction θᵣₑ꜀(t),
- differentiation rule for θ̇(t),
- memory horizon Tₘ,
- gating function w(t),
- scale parameters (σ_θ, σ_ω, σ_m),
- weights (a, b, c),
- threshold J꜀ and dwell time ΔT.
...
| Reference phase θᵣₑ꜀ | baseline/model/adaptive rule explicitly specified | locked |
...
| Thresholding (J꜀, ΔT) | alert policy pre-declared before final test | locked |
...
For λ > 0, the potential admits two metastable wells, and slow drift in λ(t) combined with delay and noise generates branch-dependent transitions and hysteretic switching. This makes the model suitable as a controlled benchmark for testing the operator under path dependence, consistent with early-warning approaches that exploit lag structure near approaching bifurcations [13].
...
2. estimating θᵣₑ꜀(t), (σ_θ, σ_ω, σ_m), (a, b, c), Tₘ, J꜀, and ΔT from calibration/validation only,
...
- lead time before branch transition or escape event, following the standard emphasis on warning-time evaluation in tipping-point detection [16],
...
In an illustrative delayed-bistable benchmark under fixed benchmark-specific parametrization, consistent with the use of predictability-based indicators for critical-transition detection [12], the simulated system underwent a regime transition at

> t_trans = 64.20 s

The benchmark exhibited a sharp transition in this particular drift–delay configuration. Under the locked-parameter evaluation, none of the tested methods achieved a positive lead time before the transition. The amplitude-threshold baseline produced no false alarms and a ROC-AUC of 0.209, while the drift-only baseline produced no false alarms and a ROC-AUC of 0.487. The ablated (U₁, U₂) variant produced one false alarm and a ROC-AUC of 0.273, whereas the full phase–memory operator produced two false alarms and a ROC-AUC of 0.153.

These results should therefore not be interpreted as evidence that the full operator outperforms simpler baselines in this specific synthetic configuration. Rather, the benchmark functions as a locked-parameter ablation and robustness check: the methods behave distinctly under identical replay conditions, and the full operator remains deterministic and numerically stable. A positive early-warning claim would require additional drift–delay regimes, broader robustness sweeps, and pre-registered test configurations showing reproducible lead-time improvement over declared baselines [16, 21].
...
| λ(t) | slow drift schedule | regime destabilisation |
| Δt | integration step | numerical replay resolution |
| Tₘ | operator memory horizon | finite hysteresis encoding |

**Table 6: Quantitative benchmark comparison for the delayed bistable simulation under the locked-parameter configuration used in Fig. 1.**
...
In this sharp transition setting, no method achieved positive lead time; the table is therefore interpreted as an ablation and robustness check rather than as evidence of benchmark-level early-warning superiority. ROC-AUC is reported here for completeness, but it is not the primary evaluation metric for a supervisory diagnostic. The main metrics are deterministic replay stability and parameter auditability (Table 3).
...
(Setpoint)    →   (Hardware)   →    (Telemetry)
                                         ↓
                               Preprocessing s(t) = P[y(t)]
                                         ↓
                               Phase θ(t), θref(t)
                                         ↓
                               Triadic Op. (U1, U2, U3)
                                         ↓
                               Stability J(t) + Dwell ΔT
                                         ↓
                               ALARM  →  Audit Log
...
2. **Reference definition:** construct θᵣₑ꜀(t) from stable segments or nominal operating envelopes.
3. **Calibration:** fit (a, b, c) and (J꜀, ΔT) on calibration window only; freeze parameters.
...
**System abstraction.** Consider a loop characterised by commanded input u(t) (e.g., valve setting or flow command) and measured output y(t) (e.g., partial oxygen pressure, flow rate, or proxy variables). Human-in-the-loop operation and cyclic breathing patterns can induce path dependence and drift in the effective timing relationship between u(t) and y(t).

**Role of the operator.** The operator is employed strictly as a supervisory diagnostic: it does not replace controllers, modify actuation laws, or introduce new physical dynamics. It provides a supervisory stability index J(t) that detects phase drift and memory-driven divergence relative to a calibrated operating regime.

**Operational outputs.** Application yields: (i) a continuous stability index J(t), (ii) identification of admissible operating windows, (iii) dwell-based supervisory alerts under calibrated conditions, and (iv) reproducible diagnostics based solely on measured time series.
...
| Alert logic | threshold-dependent | dwell-based supervisory alert via J(t) |
| Hardware changes | often required | none required in principle |

### 9.3  Illustrative Extension to Large Life-Support Environments (Conceptual)

The proposed phase–memory operator is not restricted to small closed-loop subsystems. In principle, it may also be considered for larger hybrid environments in which multiple subsystems interact with different response times, delays, and history-dependent effects, as also encountered in coupled human–environment systems with regime shifts [17, 20]. Examples include modular life-support architectures combining physical–chemical processes with slower biological or human-in-the-loop components.

Such environments may exhibit hysteresis, timing drift, and delayed cross-coupling that are not always well summarised by memoryless thresholding alone. Within the present framework, this motivates considering the phase–memory operator as an additional supervisory diagnostic layer rather than as a replacement for existing control architectures.
...
| Global coupling | Often treated locally or hierarchically | Can be summarised through an aggregated supervisory score |
...
**Energy and operational interpretation.** The present work does not establish improved energy efficiency or superior closed-loop control performance for such environments. Rather, the potential role of the operator is limited to supervisory monitoring and dwell-based alerting under delayed, path-dependent dynamics. Any claim of operational benefit would require explicit benchmark or deployment-specific validation.
...
- it yields no reproducible diagnostic or lead-time advantage over declared baselines across pre-registered test splits,
...
- the supervisory alert logic defined by (J꜀, ΔT) fails to remain stable under deterministic replay,
...
These falsification conditions are intentionally operational: they are designed to test whether the method adds diagnostic or supervisory value beyond simpler baselines while remaining reproducible under controlled evaluation, consistent with the broader literature on early-warning indicators and critical transitions in complex systems [18, 21].
...
Second, strong nonstationarity may require careful construction of the reference phase θᵣₑ꜀(t). Failure to specify the reference-phase definition, update rule, or calibration window explicitly is treated here as a methodological error rather than a hidden model degree of freedom.
...
We recommend logging: (i) the preprocessing map 𝒫 and all associated filter parameters, (ii) the reference-phase construction θᵣₑ꜀(t), (iii) the normalization scales (σ_θ, σ_ω, σ_m), (iv) the stability weights (a, b, c), (v) the alert parameters (J꜀, ΔT, Tₘ), (vi) all split indices, and (vii) artifact masks and gating definitions w(t). Deterministic replay is achieved by freezing all of the above elements and reapplying the same decision logic to the same inputs.

**Software availability (operator reference implementation).** A reference implementation of the deterministic phase–memory operator (MIT license), including deterministic replay, parameter freezing, and audit logging for reviewer verification, is available at: https://github.com/dfeen87/Phase-Memory-Operator. The repository provides reproducibility artifacts including locked-parameter examples, replay notes, and validation notebooks to support independent verification of the reported diagnostic logic. Unlike learning-based safety approaches that modify objectives or training procedures, the present work emphasises deterministic runtime supervision and exact replay [1].

---

## Appendix B  External Validation: EEG Collapse Regimes

This appendix summarises an external validation of the proposed phase–memory operator on EEG time series exhibiting collapse-like dynamics. The purpose is not parameter optimisation, but an out-of-sample robustness check using fixed operator settings and deterministic replay.
...
- event-related supervisory alerting via Eqs. (10) and (11), depending on the implementation setting.
...
[2] D. Gabor, "Theory of Communication," *Journal of the Institution of Electrical Engineers – Part III: Radio and Communication Engineering* 93(26), 429–457 (1946). doi:10.1049/ji-3-2.1946.0076.

[3] D. E. Vakman, "On the Analytic Signal, the Teager–Kaiser Energy Algorithm, and Other Methods for Defining Amplitude and Frequency," *IEEE Transactions on Signal Processing* 44(4), 791–797 (1996). doi:10.1109/78.492532.

[4] A. Visintin, *Differential Models of Hysteresis*. Springer, Berlin (1994). doi:10.1007/978-3-662-11557-2.

[5] M. Brokate and J. Sprekels, *Hysteresis and Phase Transitions*. Applied Mathematical Sciences, Vol. 121. Springer, New York (1996). doi:10.1007/978-1-4612-4048-8.

[6] C. Kuehn, "A mathematical framework for critical transitions: bifurcations, fast–slow systems and stochastic dynamics," *Physica D: Nonlinear Phenomena* 240(12), 1020–1035 (2011). doi:10.1016/j.physd.2011.02.012.

[7] M. Scheffer et al., "Early-warning signals for critical transitions," *Nature* 461(7260), 53–59 (2009). doi:10.1038/nature08227.

[8] V. Dakos et al., "Methods for Detecting Early Warnings of Critical Transitions in Time Series Illustrated Using Simulated Ecological Data," *PLoS ONE* 7(7), e41010 (2012). doi:10.1371/journal.pone.0041010.

[9] T. Saito and M. Rehmsmeier, "The Precision–Recall Plot Is More Informative than the ROC Plot When Evaluating Binary Classifiers on Imbalanced Datasets," *PLoS ONE* 10(3), e0118432 (2015). doi:10.1371/journal.pone.0041010.

[10] A. H. Shoeb, *Application of Machine Learning to Epileptic Seizure Onset Detection and Treatment*. Ph.D. thesis, Massachusetts Institute of Technology, Cambridge, MA (2009). Available at: http://dspace.mit.edu/handle/1721.1/7582.

[11] M. Krüger, D. Feeney, and M. T. Wende, "Information-Theoretic Modeling of Neural Coherence via Triadic Spiral-Time Dynamics: A Framework for Neurodynamic Collapse," *Medinformatics*, 2026. doi:10.47852/bonviewMEDIN62029043.

[12] J. Choi and P. Kim, "Early warning for critical transitions using machine-based predictability," *AIMS Mathematics* 7(11), 20313–20327 (2022). doi:10.3934/math.20221112.

[13] G. Tirabassi and C. Masoller, "Correlation lags give early warning signals of approaching bifurcations," *Chaos, Solitons & Fractals* 155, 111720 (2022). doi:10.1016/j.chaos.2021.111720.

[14] F. O. O. Wagener and A. de Zeeuw, "Stable partial cooperation in managing systems with tipping points," *Journal of Environmental Economics and Management* 109, 102499 (2021). doi:10.1016/j.jeem.2021.102499.

[15] T. M. Bury et al., "Deep learning for early warning signals of tipping points," *Proceedings of the National Academy of Sciences* 118(39), e2106140118 (2021). doi:10.1073/pnas.2106140118.

[16] A. Banerjee, I. Pavithran, and R. I. Sujith, "Early warnings of tipping in a non-autonomous turbulent reactive flow system: Efficacy, reliability, and warning times," *Chaos* 34(1), 013113 (2024). doi:10.1063/5.0160918.

[17] M. Homayounfar and R. Muneepeerakul, "On coupled dynamics and regime shifts in coupled human–water systems," *Hydrological Sciences Journal* 66(5), 769–776 (2021). doi:10.1080/02626667.2021.1883192.

[18] D. Dylewsky et al., "Universal early warning signals of phase transitions in climate systems," *Journal of the Royal Society Interface* 20(201), 20220562 (2023). doi:10.1098/rsif.2022.0562.

[19] E.-J. Kim and R. Hollerbach, "Non-equilibrium statistical properties, path-dependent information geometry, and entropy relations in edge-localized modes in fusion plasmas," *Physics of Plasmas* 29(11), 112302 (2022). doi:10.1063/5.0109257.

[20] N. G. MacLaren, P. Kundu, and N. Masuda, "Early warnings for multi-stage transitions in dynamics on networks," *Journal of the Royal Society Interface* 20(200), 20220743 (2023). doi:10.1098/rsif.2022.0743.

[21] V. Dakos et al., "Tipping point detection and early warnings in climate, ecological, and human systems," *Earth System Dynamics* 15, 1117–1135 (2024). doi:10.5194/esd-15-1117-2024.
